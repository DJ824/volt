...
class LockFreeQueueMpscSeq
{
   using AtomicSeq = std::atomic<size_t>;

   static_assert((SIZE & (SIZE - 1)) == 0, "size must be power of 2");
   static_assert(std::is_nothrow_default_constructible_v<T>, "T must be nothrow default constructible");
   ...
   [[nodiscard]]
   Slot& slot_ref(size_t index) noexcept
   {
      return slots_[slot_index(index)];
   }

   static void spin_pause() noexcept { _mm_pause(); }
...
public:
   explicit LockFreeQueueMpscSeq() noexcept
   {
   ...
   LockFreeQueueMpscSeq(const LockFreeQueueMpscSeq&)            = delete;
   LockFreeQueueMpscSeq& operator=(const LockFreeQueueMpscSeq&) = delete;

   void emplace(const T& value) noexcept { push(value); }

   void pop(T& out) noexcept
   {
      const size_t pos = consumer_.tail_.load(std::memory_order_relaxed);
      pop_direct(pos, out);
      consumer_.tail_.store(pos + 1, std::memory_order_release);
   }
   ...
   {
      const size_t pos = producer_.head_.fetch_add(1);
      push_direct(std::forward<U>(value), pos);
   }

   [[nodiscard]]
   bool try_emplace(const T& value) noexcept
   {
      for (;;)
      {
         ...
         const std::intptr_t diff     = static_cast<std::intptr_t>(observed) - static_cast<std::intptr_t>(pos);

         if (diff == 0)
         {
            if (producer_.head_.compare_exchange_weak(pos, pos + 1, std::memory_order_relaxed, std::memory_order_relaxed))
            {
               slot.val = value;
               slot.seq.store(pos + 1, std::memory_order_release);
               return true;
   ...
   bool try_pop(T& out) noexcept
   {
      ...
      Slot&               slot     = slot_ref(pos);
      const size_t        observed = slot.seq.load(std::memory_order_acquire);
      const std::intptr_t diff     = static_cast<std::intptr_t>(observed) - static_cast<std::intptr_t>(pos + 1);
   ...
   bool enqueue(const T& item) noexcept { return try_emplace(item); }
