...
template<typename T,
         size_t SIZE,
         bool   DIRECT_SLOTS =
            std::is_trivially_copyable_v<T>&& std::is_trivially_default_constructible_v<T>&& std::is_trivially_destructible_v<T>>
...
class LockFreeQueue2<T, SIZE, true>
{
   ...
   static_assert(std::is_trivially_copyable_v<T>, "requires trivially copyable T");
   static_assert(std::is_trivially_default_constructible_v<T>, "requires trivially default-constructible T");
   ...
   [[nodiscard]]
   static constexpr size_t slot_index(size_t index) noexcept
   {
...
public:
   ...
   LOCK_FREE_QUEUE2_ALWAYS_INLINE void emplace(Args&&... args) noexcept(std::is_nothrow_constructible_v<T, Args&&...>)
   {
      static_assert(std::is_constructible_v<T, Args&&...>, "[emplace] requires constructible T");
      const size_t write_index = producer_.write_index_;
   ...
   [[nodiscard]]
   LOCK_FREE_QUEUE2_ALWAYS_INLINE bool try_emplace(const T& value) noexcept
   {
      const size_t write_index      = producer_.write_index_;
   ...
   [[nodiscard]]
   LOCK_FREE_QUEUE2_ALWAYS_INLINE bool try_emplace(T&& value) noexcept
   {
      const size_t write_index      = producer_.write_index_;
      size_t       read_index_cache = producer_.read_index_cache_;
   ...
   template<typename... Args>
   [[nodiscard]]
   LOCK_FREE_QUEUE2_ALWAYS_INLINE bool try_emplace(Args&&... args) noexcept(std::is_nothrow_constructible_v<T, Args&&...>)
   {
      static_assert(std::is_constructible_v<T, Args&&...>, "requires constructible T");
      const size_t write_index      = producer_.write_index_;
      size_t       read_index_cache = producer_.read_index_cache_;
   ...
   [[nodiscard]]
   LOCK_FREE_QUEUE2_ALWAYS_INLINE bool try_pop(T& out) noexcept
   {
      const size_t read_index        = consumer_.read_index_;
...
{
   static_assert((SIZE & (SIZE - 1)) == 0, "size must be power of 2");
   static_assert(std::is_move_constructible_v<T>, "requires move-constructible T");
   static_assert(std::is_destructible_v<T>, "requires destructible T");
   ...
   [[nodiscard]]
   LOCK_FREE_QUEUE2_ALWAYS_INLINE T* slot_ptr(size_t index) noexcept
   {
      return std::launder(reinterpret_cast<T*>(std::addressof(buffer_[index])));
   }

   template<typename... Args>
   LOCK_FREE_QUEUE2_ALWAYS_INLINE void construct_slot(size_t index,
...
public:
   ...
   ~LockFreeQueue2()
   {
      size_t       read_index  = published_read_.read_index_.load(std::memory_order_relaxed);
      const size_t write_index = published_write_.write_index_.load(std::memory_order_relaxed);
   ...
   LOCK_FREE_QUEUE2_ALWAYS_INLINE void emplace(Args&&... args) noexcept(std::is_nothrow_constructible_v<T, Args&&...>)
   {
      ...
      published_write_.write_index_.store(write_index + 1, std::memory_order_release);
   }

   LOCK_FREE_QUEUE2_ALWAYS_INLINE void pop(T& out) noexcept
   {
      static_assert(std::is_move_assignable_v<T>, "[pop] requires move-assignable T");
      const size_t read_index = consumer_.read_index_;
      ...
      published_read_.read_index_.store(read_index + 1, std::memory_order_release);
   }
   ...
   LOCK_FREE_QUEUE2_ALWAYS_INLINE bool try_pop(T& out) noexcept
   {
      static_assert(std::is_move_assignable_v<T>, "requires move-assignable T");
      const size_t read_index = consumer_.read_index_;
